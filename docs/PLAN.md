...
- **Delete from a line's menu (GUI.md slice 3), the last verb of that slice.** It asks first, over
  ...
  **An obstruction (`~`) wedges the working copy, and `svn cleanup` cannot undo that.** An external
  ...
  not seen in the real app. **`sv rm` now uses them too.** It had disagreed in three places, each a
...
zero warnings, **3254 tests green** across seven test projects, status output diffed against
