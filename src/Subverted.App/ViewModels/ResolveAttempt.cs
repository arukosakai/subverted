using Subverted.Core;
using Subverted.Protocol;

namespace Subverted.App.ViewModels;

/// <summary>One resolve and what came of it, as data for whatever keeps a record of writes.</summary>
/// <param name="Target">The resolved path, relative to the root.</param>
/// <param name="Kept">Which version it was told to keep.</param>
/// <param name="Notice">What the view says about it.</param>
/// <param name="Answer">The daemon's answer as it came, or <c>null</c> when no daemon answered.</param>
public sealed record ResolveAttempt(
    string Target,
    ConflictResolution Kept,
    Notice Notice,
    DaemonResponse? Answer
);
