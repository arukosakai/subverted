...
public static class ListedLines
{
    ...
    /// <returns>
    /// For <see cref="ListedNodes.Changes"/>, the changes alone, whatever the listing held. For
    ...
    public static IReadOnlyList<ChangeRow> Of(IReadOnlyList<ChangeRow> rows, ListedNodes listed) =>
        listed == ListedNodes.All
            ? [.. rows.Where(row => !row.IsUnmodified || row.Entry.Kind != NodeKind.Directory)]
            : [.. rows.Where(row => !row.IsUnmodified)];
}
